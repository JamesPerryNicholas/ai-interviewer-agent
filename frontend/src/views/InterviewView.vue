...
import { ArrowDown, ArrowRight, ArrowUp, ChatDotRound, CircleCheck, CopyDocument, Cpu, MagicStick, Mic, RefreshLeft, UserFilled } from "@element-plus/icons-vue";
...
<template>
  <div class="page-view interview-view" @click="closeMessageMenu">
    ...
    <template v-if="!isStartMode">
      <section class="interview-header"><div class="interview-title"><div class="interview-bot-mark"><el-icon><Cpu /></el-icon></div><div><span class="eyebrow">{{ t("interview.live", { id: activeInterviewId }) }}</span><h1>{{ interviewTitle }}</h1></div></div><div class="interview-controls"><span class="live-status" :class="{ completed: isInterviewCompleted }"><i></i> {{ isInterviewCompleted ? "面试已完成" : t("interview.active") }}</span><el-button v-if="!isInterviewCompleted" type="danger" plain :loading="isFinishLoading" @click="endInterviewManually">结束面试</el-button><el-button v-if="isInterviewCompleted" type="primary" plain @click="openReportAction">{{ reportActionLabel }}</el-button><el-button text><el-icon><ArrowDown /></el-icon></el-button></div></section>
